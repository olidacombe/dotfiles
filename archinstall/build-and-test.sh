...
EOF
    ...
    jq --arg user "$USERNAME" --arg hyprconf "$(base64 -w 0 ../dot_config/hypr/hyprland.conf)" \
        '.custom_commands += ["usermod -s $(which zsh) \($user)", "echo \"set -x; script -c \\\"\\$(curl -fsLS https://github.com/olidacombe/dotfiles/raw/main/bootstrap.sh)\\\" bootstrap.log\" > /home/\($user)/.zshrc", "mkdir -p /home/\($user)/.config/hypr", "base64 -d <<< \($hyprconf) > /home/\($user)/.config/hypr/hyprland.conf", "chown -R \($user):\($user) /home/\($user)"]' \
